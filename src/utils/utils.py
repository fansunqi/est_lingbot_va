...
def warmup_linear_lambda(current_step, warmup_steps=1000, total_steps=50000,
                         min_lr_ratio=0.1):
    """Linear warmup -> linear decay to ``min_lr_ratio * base_lr``.

    Returns a multiplier on the optimizer's base lr so PyTorch's ``LambdaLR``
    can broadcast it across all param groups (e.g. Muon vs AdamW).
    """
    if current_step < warmup_steps:
        return float(current_step) / float(max(1, warmup_steps))
    progress = (current_step - warmup_steps) / float(max(1, total_steps - warmup_steps))
    progress = min(1.0, max(0.0, progress))
    return 1.0 + (float(min_lr_ratio) - 1.0) * progress