...
| `packing.max_tokens` | ✓ | - | 训练样本的序列长度上限；显存不足时优先调小 |
| `packing.max_episodes_per_bin` | ✓ | - | 单个训练样本最多拼接的轨迹片段数，默认不用改 |
| `host` | - | 仅推理 | server 绑定地址 |
| `port` | - | 仅推理 | server 绑定端口 |
| `enable_offload` | - | ✓ | 将 VAE 和 text_encoder 卸载到 CPU（节省显存） |

`packing.max_tokens` 必须是 128 的倍数。换模型、显卡、分辨率或 action stride 后，建议重新估计：
...
使用 summary 中 `Max F_probe` 或 `Best measured F` 所在行的 `total_seq` 作为 `packing.max_tokens`；如果训练仍 OOM，再适当调小。`packing.max_episodes_per_bin` 表示一个训练样本里最多拼接多少段轨迹片段，默认不用改；如果改了它，`--probe-n-episodes` 也要填同一个值。
