...
#   CONFIG_NAME=test_train bash script/run_va_posttrain_multinode.sh
#   # B) run on a hand-picked subset (one-shot override)
#   IP_LIST="ip1:8,ip2:8" CONFIG_NAME=test_train \
...
#   CONFIG_NAME=robotwin_train   training config.
...
log "MASTER=${MASTER_ADDR}:${MASTER_PORT}  CONFIG=${CONFIG_NAME}  REPO=${REPO_DIR}"
log "VENV=${VENV_PATH}"
...
export NGPU='${NGPU}' CONFIG_NAME='${CONFIG_NAME}'
