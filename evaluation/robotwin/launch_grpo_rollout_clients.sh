...
SHUFFLE_ARGS=()
if [[ "${SHUFFLE_ASSIGNMENT}" == "1" ]]; then
  SHUFFLE_ARGS+=(--shuffle_assignment_each_pass)
fi
if [[ -n "${ASSIGNMENT_SHUFFLE_SEED}" ]]; then
  SHUFFLE_ARGS+=(--assignment_shuffle_seed "${ASSIGNMENT_SHUFFLE_SEED}")
fi

echo "GRPO rollout settings: GROUP_SIZE=${GROUP_SIZE} NUM_CLIENTS=${NUM_CLIENTS} WORLD_SIZE=${WORLD_SIZE} RANK=${RANK} NUM_PASSES=${NUM_PASSES} SHUFFLE_ASSIGNMENT=${SHUFFLE_ASSIGNMENT} ASSIGNMENT_SHUFFLE_SEED=${ASSIGNMENT_SHUFFLE_SEED:-<seed>} CLIENT_RESTART_EVERY_ITEMS=${CLIENT_RESTART_EVERY_ITEMS} EVAL_ONLY=${EVAL_ONLY} CLIENT_GPUS=${CLIENT_GPUS[*]}"
...
for CLIENT_ID in $(seq 0 $((NUM_CLIENTS - 1))); do
  ...
  setsid "${PYTHON}" -u -m evaluation.robotwin.grpo_rollout_client \
    ...
    --restart_every_items "${CLIENT_RESTART_EVERY_ITEMS}" \
    "${SHUFFLE_ARGS[@]}" \
